...
sheetOverlay?.addEventListener("click", (e) => {
  ...
  /* ---------- Hero clips (read from js/config.js) ---------- */
  function normClip(c) {
    if (!c) return null;
  ...
  /* ---------- Work grid: hover-to-preview ---------- */
  if (window.matchMedia("(pointer: fine)").matches) {
    document.querySelectorAll(".work-card").forEach((card) => {
      const item = (CFG.work || {})[card.dataset.video || ""] || {};
      const thumb = card.querySelector(".work-thumb");
      if (!thumb) return;

      if (item.poster) {
        const img = document.createElement("img");
        img.className = "work-poster";
        img.src = item.poster;
        img.alt = "";
        img.loading = "lazy";
      ...
      // Only self-hosted files can preview inline; YouTube/Vimeo open in the player
      if (!item.src || toEmbed(item.src)) return;

      const v = document.createElement("video");
      v.className = "work-preview";
      v.muted = true;
      v.loop = true;
      v.playsInline = true;
      v.setAttribute("playsinline", "");
